...
public class ModelImpl implements IModel {
   ...
   *
   * @param frameNum - a  frame Number.
   * @return - A List containing IShape objects.
   */
  @Override
  public List<IShape> getShapesAtFrame(int frameNum) {
    List<IShape> frameShapes = new ArrayList<>();
    List<String> addedShapes = new ArrayList<>();

    List<IShape> shapeTemplate = this.shapes;
    if (this.recentShapes != null) {
      shapeTemplate = recentShapes;
    }
    IShape addingShape = null;
    for (IShape shape : shapeTemplate) {
      if (shape.getATime() <= frameNum && shape.getDTime() > frameNum) {
        addingShape = shape;
        for (IMotion motion : motions) {
          if (addingShape.getID().compareTo(motion.getID()) == 0 && addingShape != null) {
            addingShape = TweenShape.tweenShape(addingShape, motion, frameNum);
          }
        }
      }
      if (addingShape != null && !addedShapes.contains(addingShape.getID())) {
        frameShapes.add(addingShape);
        addedShapes.add(addingShape.getID());
      }
    }
    for (IShape shape : shapes) {
      if (shape.getATime() <= frameNum && shape.getDTime() > frameNum
              && (!addedShapes.contains(shape.getID()))) {
        addingShape = shape;
        frameShapes.add(addingShape);
        addedShapes.add(addingShape.getID());
      }
    }
    recentShapes = frameShapes;
    return frameShapes;
  }
